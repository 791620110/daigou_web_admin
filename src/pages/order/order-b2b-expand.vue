...
<BR/><BR/>
    		<Steps :current="1" direction="horizontal" v-if="row.status == '01' && this.orderData.cancelledStatus!='A'">		        
		        <Step title="订单已提交" content="订单已通过汇药网提交"></Step>
		        <Step title="订单待分仓" content="订单尚未分仓"></Step>
		    ...
		    <Steps :current="4" direction="horizontal" v-if="row.status == '05' && this.orderData.cancelledStatus!='A'">		        
		        <Step title="订单已提交" content="订单已通过汇药网提交"></Step>
		        <Step title="订单已分仓" content="订单已完成分仓"></Step>
		        <Step title="订单已出库" content="订单已完成分拣出库"></Step>
		        <Step title="订单已取货" content="订单已完成取货"></Step>
		    ...
		    <Steps :current="4" direction="horizontal" v-if="row.status == '06' && this.orderData.cancelledStatus!='A'">		        
		        <Step title="订单已提交" content="订单已通过汇药网提交"></Step>
		        <Step title="订单已分仓" content="订单已完成分仓"></Step>
		        <Step title="订单已出库" content="订单已完成分拣出库"></Step>
		        <Step title="订单已取货" content="订单已完成取货"></Step>
		    ...
		    <Steps :current="5" direction="horizontal" v-if="row.status == '05' && this.orderData.cancelledStatus=='A'" status="error">		        
		        <Step title="订单已提交" content="订单已通过汇药网提交"></Step>
		        <Step title="订单已分仓" content="订单已完成分仓"></Step>
		        <Step title="订单已出库" content="订单已完成分拣出库"></Step>
		        <Step title="订单已取货" content="订单已完成取货"></Step>
		    ...
		    <Steps :current="5" direction="horizontal" v-if="row.status == '06' && this.orderData.cancelledStatus=='A'" status="error">		        
		        <Step title="订单已提交" content="订单已通过汇药网提交"></Step>
		        <Step title="订单已分仓" content="订单已完成分仓"></Step>
		        <Step title="订单已出库" content="订单已完成分拣出库"></Step>
		        <Step title="订单已取货" content="订单已完成取货"></Step>
    ...
    export default {
        ...
        data () {
            return {
                columns: [
                    ...
                    {title: '批号',key: 'batchNo',align: 'center',render:(h,params)=>{return params.row.batchNo?params.row.batchNo:'-/-'}},
                ...
                ],distributionData:[],hasDistribution:false,hasWaybill:false,wayBillArray:[],currentWaybillList:[],orderData:{},
            }
        },
        methods: {
            waybillTrace(target){
        ...
        created(){
        	this.$api.doOrderInfo({"orderId":this._props.row.orderId,"showLoading":"Y"}).then(res => {
                if(res.status == "S"){
                    this.data = res.result.details
                    this.orderData = res.result
                }else{
                  swal('获取订单详情失败', res.result.message, 'error');
                }
            })
	            ...
	            this.$api.doOrderDistributionInfo({"orderId":this._props.row.orderId,"showLoading":"N"}).then(res => {
	                if(res.status == "S"){
	                    ...
	                    for(let i in res.result){
	                    	...
	                    	if(item.waybillCode!=null && item.waybillCode!='')
	                    		this.wayBillArray.push({whName:item.whName,waybillCode:item.waybillCode , vendor:item.logisticsVendor,vendorName:item.logisticsVendor=='jingdong'?'京东':'其它'});
	                    	item.label=(h) => {
	                    ...
	                    if(this.wayBillArray.length){
		                    ...
		                    for(var index in this.wayBillArray){
