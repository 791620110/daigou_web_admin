...
const editButton = (vm, h, currentRow, index) => {
    ...
    }, currentRow.editting ? '保存' : (currentRow.saveFail ? '重试' : '修改包裹数'));
};
...
const generationPDFButton = (vm, h, currentRow, index) =>{
  let bnTitle = '生成电子面单';
  if(currentRow.status =="02"){
  	bnTitle = '重新生成面单';
  }
//  let url = apiConfig.LOG_WAYBILL_PDF + "?waybillCode=" + currentRow.waybillCode + "&vendor=" + currentRow.vendor;
//  return  h('a', {
//		attrs: {
//			href: url,
//			type: 'button',
                    ...
                    item.render = (h, param) => {
                        ...
                        if (item.handle.length === 2) {
                            return h('div', [
                                ...
                                generationPDFButton(this, h, currentRowData, param.index)
                            ]);
