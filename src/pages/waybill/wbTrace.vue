...
<template>
 <div>
    <Breadcrumb>
      ...
      <BreadcrumbItem>跟踪物流</BreadcrumbItem>
    </Breadcrumb>
    ...
    <Form :model="formWaybillSearch" :label-width="100" style="margin-top: 20px" ref="formWaybillSearch">
        <Row>
          ...
          <Col span="8">
            <FormItem label="运单状态" >
              <Select v-model="formWaybillSearch.status" >
                <Option value="ALL">全部</Option>
                <Option value="05">物流已取件</Option>
                <Option value="06">在途</Option>
                <Option value="07">妥投</Option>
              </Select>
            </FormItem>
          </Col>
          <Col span="16">
    ...
    <Table border :columns="columns10" :data="waybillList"></Table>
        ...
        methods: {
            doSearchReset(){
                for(let field in this.formWaybillSearch){
                	if(field =='vendor'){
                		...
                		this.formWaybillSearch.status='ALL'
            		}else{
            ...
            doWaybillList(startIndex,endIndex){
                ...
                let params = {
                  ...
                  status:this.formWaybillSearch.status,
                ...
                if(res.status == "S"){
                  ...
                  for(let record of res.result.records){
                    if(record){
                	  if("04"==record.status || "03"==record.status){
                	  	record.statusLabel = "待物流取件";
                	  }
                	  if("05"==record.status){
                	  	record.statusLabel = "物流已取件";
                	  }
                	  if("06"==record.status){
                	  	record.statusLabel = "在途";
                	  }
                	  if("07"==record.status){
                	  	record.statusLabel = "妥投";
                	  }
