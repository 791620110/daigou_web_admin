...
<template>
	<Tabs :animated="false" @on-click="waybillTrace">
        <TabPane label="物流跟踪" >
        <span v-if="traceData.length ==0">暂无物流信息，请稍后再试。</span>
        <template>
    		<Timeline>
    			<TimelineItem color="green" v-for="(trace,index) in traceData" >
...
<script>
    export default {
        ...
        methods: {
            waybillTrace(target){
            	if(target == '0'){
            		this.doWaybillTrace(this._props.row.waybillCode,this._props.row.vendor);
            	}
            	if(target == '1'){
            		this.getWaybillInfo();
            	}
